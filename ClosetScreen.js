    ...
    <SafeAreaView style={styles.container}>
      ...
      {allFiltered.length === 0 ? (
        <View style={styles.empty}>
          <Feather name={laundryMode ? 'wind' : 'wind'} size={36} color={COLORS.ink3} />
          <Text style={styles.emptyTitle}>{laundryMode ? 'Laundry pile is empty' : 'Nothing here yet'}</Text>
          <Text style={styles.emptySub}>{laundryMode ? 'No items marked as in the wash' : 'Tap + to add your first item'}</Text>
        </View>
      ) : (
        <FlatList
          data={allFiltered}
          ...
          renderItem={({ item, index }) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => laundryMode ? handleMarkClean(item.id) : navigate('ItemDetail', { itemId: item.id })}
                activeOpacity={0.85}>
